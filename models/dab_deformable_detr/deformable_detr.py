...
class DABDeformableDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, outputs_class, outputs_coord = self.transformer(srcs, masks,
                                                                                                            pos,
                                                                                                            query_embeds)

        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1],'features':hs[-1]}
        if self.aux_loss:
