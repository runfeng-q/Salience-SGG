...
class BasicSceneGraphEvaluator:
    ...
    def evaluate_scene_graph_entry(
        self, gt_entry, pred_scores, iou_thresh=0.5
    ):
...
def evaluate_from_dict(
    ...
    **kwargs
    ...
    pred_to_gt, pred_5ples, rel_scores, gt_triplets = evaluate_recall(
        gt_rels,
        gt_boxes,
        gt_classes,
        pred_rels,
...
def prepare_zeroshot(zeroshot_triplets, gt_rels, gt_classes):
    sub_class, obj_class, pred_label = gt_rels[:, 0], gt_rels[:, 2], gt_rels[:, 1]
    zeroshot_idx = np.where(zeroshot_triplets[sub_class, obj_class, pred_label] < 1)
    return zeroshot_idx[0].tolist()
...
def evaluate_recall(
    ...
    return pred_to_gt, pred_5ples, relation_scores, gt_triplets
...
def _triplet(
    ...
    #if predicate_scores is None and class_scores is None:
