...
def eval_rel_results(all_results, predicate_cls_list, result_str=""):
    ...
    for im_i, res in enumerate(tqdm(all_results)):
        ...
        if res["pred_scores"] is None:
            ...
            pred_boxes = torch.as_tensor(res["pred_boxes"])
            pred_classes = torch.as_tensor(res["pred_class"])
            pred_score = torch.as_tensor(res["pred_cls_scores"])

            sbj_obj_inds = res["sbj_obj_inds"]
            ...
            demo_boxes=pred_boxes.detach()
            keep = torchvision.ops.batched_nms(demo_boxes, pred_score, pred_classes, 0.5)
            ...
            rel_pair_idx=rel_pair_idx[det_scores_inds[:,0],:]

            rel_pair_predicate=torch.as_tensor(det_labels_prd[det_scores_inds[:, 0],det_scores_inds[:,1]])
            triplets = torch.cat((rel_pair_idx, rel_pair_predicate.unsqueeze(-1)), -1)
            keep_triplet = torch.zeros_like(rel_pair_predicate)
            ...
            det_scores_inds=det_scores_inds[keep_triplet == 1][:topk]
            del keep_triplet, iou_assignments_1, iou_assignments_2, rel_pair_idx, rel_pair_predicate, triplets, demo_boxes, keep, ious, iou_assignments, keep_classes, first_indices,cum_sum, unique, idx, counts, ind_sorted
            ...
            # sbj_obj_inds=sbj_obj_inds[cand_inds]
...
class OIEvaluator:
    ...
    def update(
        ...
        result_dict["gt_sbj_boxes"]=gt_boxes[gt_relations[:,0]]
        result_dict["gt_obj_boxes"]=gt_boxes[gt_relations[:,1]]
        result_dict["gt_sbj_labels"]=gt_class[gt_relations[:,0]]
        result_dict["gt_obj_labels"]=gt_class[gt_relations[:,1]]
        result_dict["gt_prd_labels"]=gt_relations[:,2]
        ...
        sbj_obj_inds=pred_entry["sbj_obj_inds"]
        result_dict["sbj_boxes"]=pred_boxes[sbj_obj_inds[:,0]]
        result_dict["obj_boxes"]=pred_boxes[sbj_obj_inds[:,1]]
        result_dict["sbj_labels"]=pred_class[sbj_obj_inds[:,0]]
        result_dict["obj_labels"]=pred_class[sbj_obj_inds[:,1]]
        result_dict["sbj_scores"]=pred_score[sbj_obj_inds[:,0]]
        result_dict["obj_scores"]=pred_score[sbj_obj_inds[:,1]]

        result_dict['sbj_obj_inds']= pred_entry["sbj_obj_inds"]
